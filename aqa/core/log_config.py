...
所有模块使用标准 logging.getLogger() 获取 logger:
    import logging
    logger = logging.getLogger("aqa.agent.base")
"""
...
def setup_logging(debug: bool = False, config: dict | None = None) -> None:
    ...
    if debug:
        if "loggers" in cfg and "aqa" in cfg["loggers"]:
            cfg["loggers"]["aqa"]["level"] = "DEBUG"
        if "root" in cfg:
            cfg["root"]["level"] = "DEBUG"

    logging.config.dictConfig(cfg)
    logging.getLogger("aqa").debug("日志系统已初始化 (debug=%s)", debug)
