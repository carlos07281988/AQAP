...
class MessageType(str, Enum):
    """消息类型 — AQAP 协议的核心语义 (PROTOCOL.md §2)"""
...
class Topic:
    ...
    BROADCAST = "aqap:broadcast"
    AGENT_PROBE = "aqap:agent:probe"
    AGENT_JUDGE = "aqap:agent:judge"
    AGENT_REPORTER = "aqap:agent:reporter"
    SYSTEM_EVENTS = "aqap:system:events"
    PLUGIN_EVENTS = "aqap:plugin:events"
    DLQ = "aqap:dlq"

    @staticmethod
    def inbox(agent_id: str) -> str:
        return f"aqap:inbox:{agent_id}"
...
class AQAPMessage:
    ...
    def _resolve_type(raw: str) -> MessageType:
        ...
        try:
            ...
            try:
                ...
                import logging
                logging.getLogger("aqap_sdk.message").warning("未知消息类型 '%s', 标记为 UNKNOWN", raw)
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "AQAPMessage":
        return cls(
            type=data.get("type", ""),
    ...
    @classmethod
    def from_json(cls, raw: str) -> "AQAPMessage":
        return cls.from_dict(json.loads(raw))

    # ── 消息构建 ──

    def reply(self, msg_type: MessageType, payload: dict[str, Any]) -> "AQAPMessage":
        """
    ...
    @classmethod
    def task_dispatch(cls, source: str, payload: dict[str, Any]) -> "AQAPMessage":
        return cls(MessageType.TASK_DISPATCH, source, payload, topic=Topic.AGENT_PROBE)

    @classmethod
    def task_result(cls, source: str, payload: dict[str, Any]) -> "AQAPMessage":
        return cls(MessageType.TASK_RESULT, source, payload, topic=Topic.AGENT_JUDGE)

    @classmethod
    def judge_verdict(cls, source: str, payload: dict[str, Any]) -> "AQAPMessage":
        return cls(MessageType.JUDGE_VERDICT, source, payload, topic=Topic.AGENT_REPORTER)

    @classmethod
    def heartbeat(cls, source: str, status: dict[str, Any]) -> "AQAPMessage":
